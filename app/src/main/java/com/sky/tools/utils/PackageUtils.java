...
/**
 * PackageUtils
 * 类中使用的常量来自于{@link android.content.pm.PackageManager}类，目前已经同步7.1.1_r6版本安装和删除常量
 * <ul>
...
public class PackageUtils {
     ...
     * </ul>
     * <p>
     * <strong>使用下述方法获取安装的返回结果，因为是在同一线程获取另一个进程的结果，如果另一个进程一直不返回，会导致当前进程阻塞。</strong>
     * @param context
     * @param filePath file path of package
    ...
    public static int installSilent(Context context, String filePath, String pmParams) {
        ...
        if (commandResult.errorMsg.contains("INSTALL_FAILED_PERMISSION_MODEL_DOWNGRADE")) {
        ...
        if (commandResult.errorMsg.contains("INSTALL_FAILED_USER_RESTRICTED")) {
            return INSTALL_FAILED_USER_RESTRICTED;
        }
        if (commandResult.errorMsg.contains("INSTALL_FAILED_DUPLICATE_PERMISSION")) {
            return INSTALL_FAILED_DUPLICATE_PERMISSION;
        }
        if (commandResult.errorMsg.contains("INSTALL_FAILED_NO_MATCHING_ABIS")) {
            return INSTALL_FAILED_NO_MATCHING_ABIS;
        }
        if (commandResult.errorMsg.contains("NO_NATIVE_LIBRARIES")) {
            return NO_NATIVE_LIBRARIES;
        }
        if (commandResult.errorMsg.contains("INSTALL_FAILED_ABORTED")) {
            return INSTALL_FAILED_ABORTED;
        }
        if (commandResult.errorMsg.contains("INSTALL_FAILED_EPHEMERAL_INVALID")) {
            return INSTALL_FAILED_EPHEMERAL_INVALID;
        }
        return INSTALL_FAILED_OTHER;
    }
    ...
    public static int uninstallSilent(Context context, String packageName, boolean isKeepData) {
        ...
        if (commandResult.errorMsg.contains("DELETE_FAILED_INTERNAL_ERROR")) {
            return DELETE_FAILED_INTERNAL_ERROR;
        }
        if (commandResult.errorMsg.contains("DELETE_FAILED_DEVICE_POLICY_MANAGER")) {
            return DELETE_FAILED_DEVICE_POLICY_MANAGER;
        }
        if (commandResult.errorMsg.contains("DELETE_FAILED_USER_RESTRICTED")) {
            return DELETE_FAILED_USER_RESTRICTED;
        }
        if (commandResult.errorMsg.contains("DELETE_FAILED_OWNER_BLOCKED")) {
            return DELETE_FAILED_OWNER_BLOCKED;
        }
        if (commandResult.errorMsg.contains("DELETE_FAILED_ABORTED")) {
    ...
    /**
     * Installation return code: if the new package has an older version code than the currently installed package.
     */
    ...
    /**
     * Installation failed return code: if the system failed to install the
    ...
    /**
     * Deletion failed return code: if the system failed to delete the package
     ...
     */
    public static final int DELETE_FAILED_INVALID_PACKAGE = -1000001;

    /**
     * Uninstall return code<br/>
     * uninstall fail if permission denied
     */
    public static final int DELETE_FAILED_PERMISSION_DENIED = -1000002;
}
